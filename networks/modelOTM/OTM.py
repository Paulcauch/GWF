# Adapted from https://github.com/jaemoo-choi/Scalable-JKO
# (Choi et al., "Scalable Wasserstein Gradient Flow for Generative Modeling
#  through Unbalanced Optimal Transport", ICML 2024)
...
#     return self.gamma * o + x
    def forward(self, x, y=None):
        ...
        phi   = self.phi(x)                        # [B, C/8, H, W]
        g     = self.g(x)                          # [B, C/2, H, W]

        # pool 2x2 sur phi et g
        phi = F.max_pool2d(phi, kernel_size=2, stride=2)   # [B, C/8, Hp, Wp]
        g   = F.max_pool2d(g,   kernel_size=2, stride=2)   # [B, C/2, Hp, Wp]

        Hp, Wp = phi.shape[2], phi.shape[3]        

        theta = theta.view(B, self.ch // 8, H * W)         # [B, C/8, HW]
        phi   =   phi.view(B, self.ch // 8, Hp * Wp)       # [B, C/8, HpWp]
        g     =     g.view(B, self.ch // 2, Hp * Wp)       # [B, C/2, HpWp]

        beta = F.softmax(torch.bmm(theta.transpose(1, 2), phi), dim=-1)   # [B, HW, HpWp]
        o    = torch.bmm(g, beta.transpose(1, 2)).view(B, self.ch // 2, H, W)
...
class Generator_otm(nn.Module):
    def __init__(self, config):
        super(Generator_otm, self).__init__()
        ...
        for ups in (3, 2, 1, 0):
            if config.imageSize % (2**ups) == 0:
                break
        self.ups = ups
        self.init_size = config.imageSize // (2 ** self.ups)  # exact
        ...
        for _ in range(self.ups):
            blocks.append(ResBlockGenerator(config, ngf, ngf, stride=2))
...
class Discriminator_otm(BaseDiscriminator):
    def __init__(self, config):
        super(Discriminator_otm, self).__init__(config)
        self.config = config
        ndf = config.ngf
        linear_layer = linear_layers[config.D_linear]
...
# class Discriminator_unet(BaseDiscriminator):
#     def __init__(self, config):
...
#         self.feat_net = nn.Sequential(
#             FirstResBlockDiscriminator(config, config.nc, ndf, stride=2),  # 32->16
#             ResBlockDiscriminator(config, ndf, ndf, stride=2),             # 16->8
#             Attention(ndf),                                                # <--- optionnel: SA à 16x16/8x8
#             ResBlockDiscriminator(config, ndf, ndf),                       # 8->8
#             ResBlockDiscriminator(config, ndf, ndf),
...
#         h = self.feat_net(x)          # [B, C, H, W]
#         h = h.sum((2, 3))             # global sum pool
#         out = self.final_layer(h)     # [B,1]
#         return out
...
class Discriminator_unet(BaseDiscriminator):
    def __init__(self, config):
        super().__init__(config)
        self.config = config
        ndf = getattr(config, "ndf", config.ngf)          # <-- décorrélé de ngf
        linear_layer = linear_layers[config.D_linear]
        H = int(getattr(config, "image_size", 32))         # 28 (MNIST) ou 32 (CIFAR)
        C_in = config.nc
        ...
        # 1)  H -> H/2, C: nc -> ndf
        blocks += [FirstResBlockDiscriminator(config, C_in, ch, stride=2)]
        H //= 2                                            # 32->16 / 28->14

        # 2)  H/2 -> H/4, C: ndf -> 2*ndf
        blocks += [ResBlockDiscriminator(config, ch, 2*ch, stride=2)]
        ch *= 2
        H //= 2                                            # 16->8 / 14->7

        # Attention à cette échelle (8x8/7x7) — OK grâce au patch
        blocks += [Attention(ch)]

        # 3)  H/4 -> H/8 (~4 ou 3), C: 2*ndf -> 4*ndf
        blocks += [ResBlockDiscriminator(config, ch, 2*ch, stride=2)]
        ch *= 2                                            # ch = 4*ndf
        H //= 2                                            # 8->4 / 7->3

        # Un bloc résiduel en plus pour raffiner
        blocks += [ResBlockDiscriminator(config, ch, ch)]
        blocks += [nn.LeakyReLU(0.2, inplace=True)]

        self.feat_net = nn.Sequential(*blocks)
        self.final_layer = linear_layer(ch, 1)             # ch = 4*ndf
        self.final_layer.apply(weights_init_xavier)

    def forward(self, x, label=None):
        h = self.feat_net(x)       # [B, ch, H', W'] (H' = 4 ou 3)
        h = h.sum((2, 3))          # global sum pool -> [B, ch]
        return self.final_layer(h) # [B, 1]
